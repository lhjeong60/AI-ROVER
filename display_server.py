    ...
    def _run_display_stats(self):
        ...
        while self.stats_enabled:
            ...
            if (self.ina != None):
                ...
                self.draw.text((4, top), power_mode_str + (" %.1fV") % bus_voltage + (" %.2fA") % (current / 1000) + (
                    " %2.0f%%") % p, font=self.font, fill=255)
                ...
                if (p > 100): p = 100
                if (p < 0): p = 0
                self.draw.text((4, top), 'MODE: ' + power_mode_str + ("  %.1fV") % value + ("  %2.0f%%") % p,
